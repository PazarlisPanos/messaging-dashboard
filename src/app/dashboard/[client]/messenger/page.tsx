...
import { getFbConversations } from '@/lib/queries'
import { notFound } from 'next/navigation'
import InboxView from '@/components/inbox/InboxView'

export const revalidate = 30

export default async function ClientMessengerPage({ params }: { params: { client: string } }) {
  const client = await getClient(params.client)
  if (!client) notFound()
  const conversations = await getFbConversations(client.database_url)
